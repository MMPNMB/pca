...
# 6 "kernel/rhino/core/k_task.c" 2
                           ...
                           uint8_t mm_alloc_flag, uint8_t cpu_num, uint8_t cpu_binded)
{
    ...
    do { if (task == 
# 17 "kernel/rhino/core/k_task.c" 3 4
   ((void *)0)
# 17 "kernel/rhino/core/k_task.c"
   ) { return RHINO_NULL_PTR; } } while (0);
    do { if (name == 
# 18 "kernel/rhino/core/k_task.c" 3 4
   ((void *)0)
# 18 "kernel/rhino/core/k_task.c"
   ) { return RHINO_NULL_PTR; } } while (0);
    do { if (entry == 
# 19 "kernel/rhino/core/k_task.c" 3 4
   ((void *)0)
# 19 "kernel/rhino/core/k_task.c"
   ) { return RHINO_NULL_PTR; } } while (0);
    do { if (stack_buf == 
# 20 "kernel/rhino/core/k_task.c" 3 4
   ((void *)0)
# 20 "kernel/rhino/core/k_task.c"
   ) { return RHINO_NULL_PTR; } } while (0);
    ...
    do { if (g_intrpt_nested_level[cpu_cur_get()] > 0u) { do { { cpu_intrpt_restore(cpsr); }; } while (0); return RHINO_NOT_CALLED_BY_INTRPT; } } while (0);
...
# 99 "kernel/rhino/core/k_task.c"
    ...
    if (autorun > 0u) {
        ...
        if (g_sys_stat == RHINO_RUNNING) {
            do { { cpu_intrpt_restore(cpsr); }; core_sched(); } while (0);
...
# 183 "kernel/rhino/core/k_task.c"
                        ...
                        uint8_t cpu_num, uint8_t cpu_binded, uint8_t autorun)
{
    ...
    do { if (task == 
# 191 "kernel/rhino/core/k_task.c" 3 4
   ((void *)0)
# 191 "kernel/rhino/core/k_task.c"
   ) { return RHINO_NULL_PTR; } } while (0);
    ...
    if (task_stack == 
# 198 "kernel/rhino/core/k_task.c" 3 4
                     ((void *)0)
# 198 "kernel/rhino/core/k_task.c"
                         ) {
        return RHINO_NO_MEM;
    }
    ...
    if (task_obj == 
# 203 "kernel/rhino/core/k_task.c" 3 4
                   ((void *)0)
# 203 "kernel/rhino/core/k_task.c"
                       ) {
    ...
    *task = task_obj;
    ...
    ret = task_create(task_obj, name, arg, pri, ticks, task_stack, stack, entry,
                      autorun, 2u, cpu_num, cpu_binded);
    if ((ret != RHINO_SUCCESS) && (ret != RHINO_STOPPED)) {
        krhino_mm_free(task_stack);
        krhino_mm_free(task_obj);
        *task = 
# 215 "kernel/rhino/core/k_task.c" 3 4
               ((void *)0)
# 215 "kernel/rhino/core/k_task.c"
                   ;
        return ret;
    }
...
# 239 "kernel/rhino/core/k_task.c"
...
kstat_t task_suspend(ktask_t *task)
{
    ...
    cur_cpu_num = cpu_cur_get();
# 326 "kernel/rhino/core/k_task.c"
...
{
    if (task == 
# 372 "kernel/rhino/core/k_task.c" 3 4
               ((void *)0)
# 372 "kernel/rhino/core/k_task.c"
                   ) {
...
{
    do { if (task == 
# 436 "kernel/rhino/core/k_task.c" 3 4
   ((void *)0)
# 436 "kernel/rhino/core/k_task.c"
   ) { return RHINO_NULL_PTR; } } while (0);
...
kstat_t krhino_task_stack_min_free(ktask_t *task, size_t *free)
{
    ...
    do { if (task == 
# 446 "kernel/rhino/core/k_task.c" 3 4
   ((void *)0)
# 446 "kernel/rhino/core/k_task.c"
   ) { return RHINO_NULL_PTR; } } while (0);
    do { if (free == 
# 447 "kernel/rhino/core/k_task.c" 3 4
   ((void *)0)
# 447 "kernel/rhino/core/k_task.c"
   ) { return RHINO_NULL_PTR; } } while (0);
...
# 466 "kernel/rhino/core/k_task.c"
    *free = free_stk;
...
kstat_t krhino_task_stack_cur_free(ktask_t *task, size_t *free)
{
    ...
    if (task == 
# 479 "kernel/rhino/core/k_task.c" 3 4
               ((void *)0) 
# 479 "kernel/rhino/core/k_task.c"
                    || task == g_active_task[cpu_cur_get()]) {
        task = g_active_task[cpu_cur_get()];
        sp = soc_get_cur_sp();
    } else {
        ...
        if (task->prio != new_pri) {
            switch (task->task_state) {
                case K_RDY:
                    ...
                    task = 
# 526 "kernel/rhino/core/k_task.c" 3 4
                          ((void *)0)
# 526 "kernel/rhino/core/k_task.c"
                              ;
                    break;
                case K_SLEEP:
                case K_SUSPENDED:
                case K_SLEEP_SUSPENDED:
                    ...
                    task->prio = new_pri;
                    task = 
# 533 "kernel/rhino/core/k_task.c" 3 4
                          ((void *)0)
# 533 "kernel/rhino/core/k_task.c"
                              ;
                    break;
                case K_PEND:
                case K_PEND_SUSPENDED:
                    ...
                    if (task->blk_obj->obj_type == RHINO_MUTEX_OBJ_TYPE) {
                        ...
                        if (mutex_task->prio > task->prio) {
                            ...
                            if (new_pri != mutex_task->prio) {
                                ...
                                task = 
# 558 "kernel/rhino/core/k_task.c" 3 4
                                      ((void *)0)
# 558 "kernel/rhino/core/k_task.c"
                                          ;
                            }
                        } else {
                            task = 
# 561 "kernel/rhino/core/k_task.c" 3 4
                                  ((void *)0)
# 561 "kernel/rhino/core/k_task.c"
                                      ;
                        }
                    } else {
                        task = 
# 564 "kernel/rhino/core/k_task.c" 3 4
                              ((void *)0)
# 564 "kernel/rhino/core/k_task.c"
                                  ;
                    }
            ...
            task = 
# 573 "kernel/rhino/core/k_task.c" 3 4
                  ((void *)0)
# 573 "kernel/rhino/core/k_task.c"
                      ;
        }
    } while (task != 
# 575 "kernel/rhino/core/k_task.c" 3 4
                    ((void *)0)
# 575 "kernel/rhino/core/k_task.c"
                        );
...
kstat_t krhino_task_pri_change(ktask_t *task, uint8_t pri, uint8_t *old_pri)
{
    ...
    do { if (task == 
# 587 "kernel/rhino/core/k_task.c" 3 4
   ((void *)0)
# 587 "kernel/rhino/core/k_task.c"
   ) { return RHINO_NULL_PTR; } } while (0);
    do { if (old_pri == 
# 588 "kernel/rhino/core/k_task.c" 3 4
   ((void *)0)
# 588 "kernel/rhino/core/k_task.c"
   ) { return RHINO_NULL_PTR; } } while (0);
...
kstat_t krhino_task_wait_abort(ktask_t *task)
{
    ...
    do { if (task == 
# 637 "kernel/rhino/core/k_task.c" 3 4
   ((void *)0)
# 637 "kernel/rhino/core/k_task.c"
   ) { return RHINO_NULL_PTR; } } while (0);
    ...
    switch (task->task_state) {
        ...
        case K_PEND:
            ...
            task->blk_obj = 
# 671 "kernel/rhino/core/k_task.c" 3 4
                           ((void *)0)
# 671 "kernel/rhino/core/k_task.c"
                               ;
...
static void task_mutex_free(ktask_t *task)
{
    ...
    while ((mutex = next_mutex) != 
# 701 "kernel/rhino/core/k_task.c" 3 4
                                  ((void *)0)
# 701 "kernel/rhino/core/k_task.c"
                                      ) {
        ...
        if (!is_klist_empty(blk_list_head)) {
            next_task = ((ktask_t *)((uint8_t *)(blk_list_head->next) - (size_t)(&((ktask_t *)0)->task_list)));
            ...
            mutex->mutex_task = 
# 718 "kernel/rhino/core/k_task.c" 3 4
                               ((void *)0)
# 718 "kernel/rhino/core/k_task.c"
                                   ;
        }
    }
}
...
kstat_t krhino_task_del(ktask_t *task)
{
    ...
    if (task == 
# 738 "kernel/rhino/core/k_task.c" 3 4
               ((void *)0)
# 738 "kernel/rhino/core/k_task.c"
                   ) {
    ...
    if (task->mm_alloc_flag != 1u) {
        do { { cpu_intrpt_restore(cpsr); }; } while (0);
        return RHINO_KOBJ_DEL_ERR;
    }
# 761 "kernel/rhino/core/k_task.c"
    if (task == g_active_task[cpu_cur_get()]) {
    ...
    ;
# 814 "kernel/rhino/core/k_task.c"
    do { { cpu_intrpt_restore(cpsr); }; core_sched(); } while (0);
...
kstat_t krhino_task_dyn_del(ktask_t *task)
{
    ...
    if (task == 
# 834 "kernel/rhino/core/k_task.c" 3 4
               ((void *)0)
# 834 "kernel/rhino/core/k_task.c"
                   ) {
        task = g_active_task[cur_cpu_num];
    }
    ...
    if (task->mm_alloc_flag != 2u) {
        do { { cpu_intrpt_restore(cpsr); }; } while (0);

        return RHINO_KOBJ_DEL_ERR;
    }
# 859 "kernel/rhino/core/k_task.c"
    if (task == g_active_task[cpu_cur_get()]) {
        if (g_sched_lock[cpu_cur_get()] > 0u) {
            do { { cpu_intrpt_restore(cpsr); }; } while (0);
            return RHINO_SCHED_DISABLE;
...
kstat_t krhino_task_time_slice_set(ktask_t *task, size_t slice)
{
    ...
    do { if (task == 
# 943 "kernel/rhino/core/k_task.c" 3 4
   ((void *)0)
# 943 "kernel/rhino/core/k_task.c"
   ) { return RHINO_NULL_PTR; } } while (0);
...
kstat_t krhino_sched_policy_set(ktask_t *task, uint8_t policy)
{
    ...
    do { if (task == 
# 968 "kernel/rhino/core/k_task.c" 3 4
   ((void *)0)
# 968 "kernel/rhino/core/k_task.c"
   ) { return RHINO_NULL_PTR; } } while (0);
...
kstat_t krhino_sched_policy_get(ktask_t *task, uint8_t *policy)
{
    ...
    do { if (task == 
# 988 "kernel/rhino/core/k_task.c" 3 4
   ((void *)0)
# 988 "kernel/rhino/core/k_task.c"
   ) { return RHINO_NULL_PTR; } } while (0);
    do { if (policy == 
# 989 "kernel/rhino/core/k_task.c" 3 4
   ((void *)0)
# 989 "kernel/rhino/core/k_task.c"
   ) { return RHINO_NULL_PTR; } } while (0);
...
kstat_t krhino_task_info_set(ktask_t *task, size_t idx, void *info)
{
    ...
    do { if (task == 
# 1007 "kernel/rhino/core/k_task.c" 3 4
   ((void *)0)
# 1007 "kernel/rhino/core/k_task.c"
   ) { return RHINO_NULL_PTR; } } while (0);
...
{
    do { if (task == 
# 1022 "kernel/rhino/core/k_task.c" 3 4
   ((void *)0)
# 1022 "kernel/rhino/core/k_task.c"
   ) { return RHINO_NULL_PTR; } } while (0);
    do { if (info == 
# 1023 "kernel/rhino/core/k_task.c" 3 4
   ((void *)0)
# 1023 "kernel/rhino/core/k_task.c"
   ) { return RHINO_NULL_PTR; } } while (0);
...
void krhino_task_deathbed(void)
{
    ...
    if (task->mm_alloc_flag == 2u) {
        ...
        krhino_task_dyn_del(
# 1045 "kernel/rhino/core/k_task.c" 3 4
                           ((void *)0)
# 1045 "kernel/rhino/core/k_task.c"
                               );

    } else {
        krhino_task_del(
# 1048 "kernel/rhino/core/k_task.c" 3 4
                       ((void *)0)
# 1048 "kernel/rhino/core/k_task.c"
                           );
    }
