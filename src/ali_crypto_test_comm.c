# 1 "security/alicrypto/./libalicrypto/test/ali_crypto_test_comm.c"
...
# 6 "security/alicrypto/./libalicrypto/test/ali_crypto_test_comm.c" 2
...
void ali_crypto_print_data(const char *name, uint8_t *data, size_t size)
{
     ...
     if (data == 
# 11 "security/alicrypto/./libalicrypto/test/ali_crypto_test_comm.c" 3 4
                ((void *)0) 
# 11 "security/alicrypto/./libalicrypto/test/ali_crypto_test_comm.c"
                     || size == 0) {
          printf("E %s %d: ""print_data: no data\n", __FUNCTION__, 12);
          return;
     }

    printf("I %s %d: ""%s size: %d\n", __FUNCTION__, 16, name, (int)size);
    for (i = 0; i < size - size % 8; i += 8) {
        printf("I %s %d: ""%s data: %02x%02x %02x%02x %02x%02x %02x%02x\n", __FUNCTION__,


                                                             21
# 18 "security/alicrypto/./libalicrypto/test/ali_crypto_test_comm.c"
        , name, data[i+0], data[i+1], data[i+2], data[i+3], data[i+4], data[i+5], data[i+6], data[i+7])


                                                              ;
    }
    while(i < size) {
        printf("I %s %d: ""%s data: %02x\n", __FUNCTION__, 24, name, data[i]);
        i++;
    }
