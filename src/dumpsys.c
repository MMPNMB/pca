...
uint32_t dumpsys_task_func(char *buf, uint32_t len, int detail)
{
    ...
    char yes = 'N';
    char *printbuf = 
# 55 "tools/cli/dumpsys.c" 3 4
                    ((void *)0)
# 55 "tools/cli/dumpsys.c"
                        ;
    char tmpbuf[256] = {0};
    int offset = 0;
    int totallen = 2048;
    ...
    if (printbuf == 
# 62 "tools/cli/dumpsys.c" 3 4
                    ((void *)0)
# 62 "tools/cli/dumpsys.c"
                        ) {
        return RHINO_NO_MEM;
    }
...
# 77 "tools/cli/dumpsys.c"
    ...
    snprintf(tmpbuf, 255,
            "%s------------------------------------------------------------------------\r\n",
            esc_tag);
    do { if ((totallen - offset) <= strlen(tmpbuf)) { csp_printf("%s",printbuf); offset = 0; } snprintf(printbuf+offset,strlen(tmpbuf)+1,"%s",tmpbuf); offset += strlen(tmpbuf); } while (0);
# 103 "tools/cli/dumpsys.c"
    snprintf(tmpbuf, 255,
    ...
    task = ((ktask_t *)((uint8_t *)(tmp) - (size_t)(&((ktask_t *)0)->task_stats_item)));
        ...
        if (task->task_name != 
# 127 "tools/cli/dumpsys.c" 3 4
                              ((void *)0)
# 127 "tools/cli/dumpsys.c"
                                  ) {
            task_name = task->task_name;
        } else {
            task_name = "anonym";
        }
# 146 "tools/cli/dumpsys.c"
        if (candidate == task)
        {
            yes = 'Y';
        } else {
                                    ...
                                    int argc, char **argv)
{
    ...
    sys_time_t round_sec = 10*1000;

    if (argc > 2 && 0 == strcmp(argv[2], "start")) {
        if (0 == run_flag) {
            if ( argc > 3 && 
# 273 "tools/cli/dumpsys.c" 3 4
                            ((void *)0) 
# 273 "tools/cli/dumpsys.c"
                                 != argv[3]) {
            ...
            krhino_timer_create(&g_mm_leak_check_timer, "mm_leak_check_timer",
                                ...
                                10, krhino_ms_to_ticks(round_sec), 
# 279 "tools/cli/dumpsys.c" 3 4
                                                                  ((void *)0)
# 279 "tools/cli/dumpsys.c"
                                                                      , 0);
        } else {
            if (
# 281 "tools/cli/dumpsys.c" 3 4
               ((void *)0) 
# 281 "tools/cli/dumpsys.c"
                    != argv[3]) {
                round_sec = atoi(argv[3]) * 1000;
                      ...
                      char **argv)
{
    ...
    if (argc >= 2 && 0 == strcmp(argv[1], "task")) {
        if (argc == 3 && (0 == strcmp(argv[2], "detail"))) {
            ret = dumpsys_task_func(pcWriteBuffer, xWriteBufferLen, 
# 312 "tools/cli/dumpsys.c" 3 4
                                                                   1
# 312 "tools/cli/dumpsys.c"
                                                                       );
        } else {
            ret = dumpsys_task_func(pcWriteBuffer, xWriteBufferLen, 
# 314 "tools/cli/dumpsys.c" 3 4
                                                                   0
# 314 "tools/cli/dumpsys.c"
                                                                        );
        }
        ...
        ret = dumpsys_mm_leak_func(
# 344 "tools/cli/dumpsys.c" 3 4
                                  ((void *)0)
# 344 "tools/cli/dumpsys.c"
                                      , 0);
        return ret;
...
int dump_task_stack(ktask_t *task)
{
    ...
    char *printbuf = 
# 387 "tools/cli/dumpsys.c" 3 4
                    ((void *)0)
# 387 "tools/cli/dumpsys.c"
                        ;
    char tmpbuf[256] = {0};
    int bufoffset = 0;
    int totallen = 2048;
    ...
    printbuf = aos_malloc(totallen);
    if (printbuf == 
# 393 "tools/cli/dumpsys.c" 3 4
                    ((void *)0)
# 393 "tools/cli/dumpsys.c"
                        ) {
        return RHINO_NO_MEM;
    }
    memset(printbuf, 0, totallen);
    ...
    while (p < (int *)end) {
        if (i % 4 == 0) {
            ...
            do { if ((totallen - bufoffset) <= strlen(tmp)) { csp_printf("%s",printbuf); bufoffset = 0; } snprintf(printbuf+bufoffset,strlen(tmp)+1,"%s",tmp); bufoffset += strlen(tmp); } while (0);
        }
...
{
    dumpsys_task_func(
# 456 "tools/cli/dumpsys.c" 3 4
                     ((void *)0)
# 456 "tools/cli/dumpsys.c"
                         , 0, 1);
}
