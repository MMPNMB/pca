...
# 6 "kernel/rhino/core/k_tick.c" 2
...
static inline void tick_list_pri_insert(klist_t *head, ktask_t *task)
{
    tick_t val;
    klist_t *q;
    klist_t *list_start;
    ...
    for (q = list_start->next; q != list_end; q = q->next) {
        task_iter_temp = ((ktask_t *)((uint8_t *)(q) - (size_t)(&((ktask_t *)0)->tick_list)));
...
void tick_list_rm(ktask_t *task)
{
    ...
    if (tick_head_ptr != 
# 51 "kernel/rhino/core/k_tick.c" 3 4
                        ((void *)0)
# 51 "kernel/rhino/core/k_tick.c"
                            ) {
        klist_rm(&task->tick_list);
        task->tick_head = 
# 53 "kernel/rhino/core/k_tick.c" 3 4
                         ((void *)0)
# 53 "kernel/rhino/core/k_tick.c"
                             ;
    }
}
...
void tick_list_update(tick_i_t ticks)
{
    ...
    while (1u) {
        ...
        if (iter != tick_head_ptr) {
            iter_temp = iter->next;
            p_tcb = ((ktask_t *)((uint8_t *)(iter) - (size_t)(&((ktask_t *)0)->tick_list)));
            ...
            if (delta <= 0) {
                switch (p_tcb->task_state) {
                    ...
                    case K_PEND:
                        ...
                        p_tcb->blk_obj = 
# 97 "kernel/rhino/core/k_tick.c" 3 4
                                           ((void *)0)
# 97 "kernel/rhino/core/k_tick.c"
                                               ;
                        break;
                    case K_PEND_SUSPENDED:
                        ...
                        mutex_task_pri_reset(p_tcb);
                        p_tcb->blk_obj = 
# 106 "kernel/rhino/core/k_tick.c" 3 4
                                           ((void *)0)
# 106 "kernel/rhino/core/k_tick.c"
                                               ;
                        break;
