...
# 6 "kernel/rhino/core/k_pend.c" 2

static inline void pend_list_add(klist_t *head, ktask_t *task)
{
    klist_t *tmp;
    ...
    for (tmp = list_start->next; tmp != list_end; tmp = tmp->next) {
        if (((ktask_t *)((uint8_t *)(tmp) - (size_t)(&((ktask_t *)0)->task_list)))->prio > task->prio) {
            break;
        }
    }
...
void pend_task_wakeup(ktask_t *task)
{
    ...
    task->blk_obj = 
# 47 "kernel/rhino/core/k_pend.c" 3 4
                     ((void *)0)
# 47 "kernel/rhino/core/k_pend.c"
                         ;
}
...
void pend_task_rm(ktask_t *task)
{
    ...
    task->blk_obj = 
# 98 "kernel/rhino/core/k_pend.c" 3 4
                   ((void *)0)
# 98 "kernel/rhino/core/k_pend.c"
                       ;
}
