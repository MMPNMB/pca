# 1 "kernel/protocols/net/port/sys_arch.c"
# 1 "/home/stone/Documents/Ali_IOT//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "kernel/protocols/net/port/sys_arch.c"
# 34 "kernel/protocols/net/port/sys_arch.c"
...
# 35 "kernel/protocols/net/port/sys_arch.c" 2
...
# 38 "kernel/protocols/net/port/sys_arch.c" 2
...
# 39 "kernel/protocols/net/port/sys_arch.c" 2
...
# 40 "kernel/protocols/net/port/sys_arch.c" 2
...
# 41 "kernel/protocols/net/port/sys_arch.c" 2


static aos_mutex_t sys_arch_mutex;
# 56 "kernel/protocols/net/port/sys_arch.c"
err_t sys_sem_new(sys_sem_t *sem, u8_t count)
{
    err_t ret = ERR_MEM;
...
{
    if ((sem != 
# 75 "kernel/protocols/net/port/sys_arch.c" 3 4
               ((void *)0)
# 75 "kernel/protocols/net/port/sys_arch.c"
                   )) {
        aos_sem_free(sem);
    }
}
# 87 "kernel/protocols/net/port/sys_arch.c"
void sys_sem_signal(sys_sem_t *sem)
{
    aos_sem_signal(sem);
}
# 109 "kernel/protocols/net/port/sys_arch.c"
u32_t sys_arch_sem_wait(sys_sem_t *sem, u32_t timeout)
{
    ...
    if (sem == 
# 114 "kernel/protocols/net/port/sys_arch.c" 3 4
              ((void *)0)
# 114 "kernel/protocols/net/port/sys_arch.c"
                  )
        return 0xffffffffUL;
    ...
    if( timeout != 0UL ) {
        ...
        while( !(aos_sem_wait(sem, 0xffffffffu) == 0));
        end_ms = sys_now();
...
# 385 "kernel/protocols/net/port/sys_arch.c"
err_t sys_mbox_new(sys_mbox_t *mb, int size)
{
    ...
    if (msg_start == 
# 391 "kernel/protocols/net/port/sys_arch.c" 3 4
                    ((void *)0)
# 391 "kernel/protocols/net/port/sys_arch.c"
                        ) {
        return ERR_MEM;
    }
...
void sys_mbox_free(sys_mbox_t *mb)
{
    ...
    if ((mb != 
# 413 "kernel/protocols/net/port/sys_arch.c" 3 4
              ((void *)0)
# 413 "kernel/protocols/net/port/sys_arch.c"
                  )) {
        start = aos_queue_buf_ptr(mb);
        if(start != 
# 415 "kernel/protocols/net/port/sys_arch.c" 3 4
                   ((void *)0)
# 415 "kernel/protocols/net/port/sys_arch.c"
                       )
            free(start);
        aos_queue_free(mb);
    }
...
# 461 "kernel/protocols/net/port/sys_arch.c"
u32_t sys_arch_mbox_fetch(sys_mbox_t *mb, void **msg, u32_t timeout)
{
    ...
    if (mb == 
# 467 "kernel/protocols/net/port/sys_arch.c" 3 4
             ((void *)0)
# 467 "kernel/protocols/net/port/sys_arch.c"
                 )
        return 0xffffffffUL;
    ...
    if( timeout != 0UL ) {
        ...
        while(aos_queue_recv(mb,0xffffffffu,msg,&len) != 0);
        end_ms = sys_now();
        elapsed_ms = end_ms - begin_ms;
...
void sys_mutex_lock(sys_mutex_t *mutex)
{
    aos_mutex_lock(mutex,0xffffffffu);
}
...
# 599 "kernel/protocols/net/port/sys_arch.c"
sys_prot_t sys_arch_protect(void)
{
    aos_mutex_lock(&sys_arch_mutex, 0xffffffffu);
    return 0;
}
