...
static const struct cli_command *lookup_command(char *name, int len)
{
    ...
    while (i < 64 && n < cli->num_commands) {
        if (cli->commands[i]->name == 
# 46 "tools/cli/cli.c" 3 4
                                     ((void *)0)
# 46 "tools/cli/cli.c"
                                         ) {
            i++;
...
# 95 "tools/cli/cli.c"
    ...
    memset(cli->outbuf, 0, 2048);

    command->function(cli->outbuf, 2048, argc, argv);
...
static int handle_input(char *inbuf)
{
    ...
    } while (!stat.done && ++i < 256 && cmdnum < 6 && (*pargc) < 16);

    if (stat.inQuote) {
        return 2;
    }

    for( i = 0; i <= cmdnum && i < 6 ; i++ )
    {
...
# 239 "tools/cli/cli.c"
    ...
    for (i = 0, n = 0, m = 0; i < 64 && n < cli->num_commands;
         i++) {
        if (cli->commands[i]->name != 
# 246 "tools/cli/cli.c" 3 4
                                     ((void *)0)
# 246 "tools/cli/cli.c"
                                         ) {
...
# 342 "tools/cli/cli.c"
            ...
            if (key2 == 0x41) {
                ...
                cli->his_idx = (cli->his_idx + 5 - 1) % 5;
                strncpy(inbuf, cmd, 256);
                csp_printf("\r\n" "# " "%s", inbuf);
                *bp = strlen(inbuf);
                esc_tag[0] = '\x0';
                esc_tag_len = 0;
...
# 455 "tools/cli/cli.c"
        ...
        if (get_input(cli->inbuf, &cli->bp)) {
            ...
            if (strlen(cli->inbuf) > 0) {
                strncpy(cli->history[cli->his_cur], cli->inbuf, 256);
                cli->his_cur = (cli->his_cur + 1) % 5;
            }
...
static const struct cli_command built_ins[] = {
    ...
    {"help", 
# 501 "tools/cli/cli.c" 3 4
                   ((void *)0)
# 501 "tools/cli/cli.c"
                       , help_cmd},
    {"echo", 
# 502 "tools/cli/cli.c" 3 4
                   ((void *)0)
# 502 "tools/cli/cli.c"
                       , echo_cmd},
...
static void help_cmd(char *buf, int len, int argc, char **argv)
{
    ...
    for (i = 0, n = 0; i < 64 && n < cli->num_commands; i++) {
        if (cli->commands[i]->name) {
            aos_cli_printf("%s: %s\r\n", cli->commands[i]->name,
                           cli->commands[i]->help ?
                           cli->commands[i]->help : "");
...
int aos_cli_register_command(const struct cli_command *cmd)
{
    ...
    if (!cmd->name || !cmd->function) {
        return -
# 618 "tools/cli/cli.c" 3
               22
# 618 "tools/cli/cli.c"
                     ;
    }

    if (cli->num_commands < 64) {
...
# 641 "tools/cli/cli.c"
    ...
    for (i = 0; i < cli->num_commands; i++) {
        if (cli->commands[i] == cmd) {
            ...
            cli->commands[cli->num_commands] = 
# 652 "tools/cli/cli.c" 3 4
                                              ((void *)0)
# 652 "tools/cli/cli.c"
                                                  ;
            return 0;
...
int aos_cli_init(void)
{
    ...
    if (cli == 
# 706 "tools/cli/cli.c" 3 4
              ((void *)0)
# 706 "tools/cli/cli.c"
                  ) {
        return -
# 707 "tools/cli/cli.c" 3
               12
# 707 "tools/cli/cli.c"
                     ;
    }
    ...
    ret = aos_task_new_ext(&task, "cli", cli_main, 0, 2048, 32 + 1);
    if (ret != 0) {
...
int cli_putstr(char *msg)
{
    ...
    if (msg[0] != 0) {
        hal_uart_send(&uart_stdio, (void *)msg, strlen(msg), 0xFFFFFFFFU);
    }
