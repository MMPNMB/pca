...
# 90 "./framework/netmgr/include/netmgr.h"
                     autoconfig);
# 14 "example/tls/tls_client.c" 2

# 1 "./include/aos/mbedtls_ssl.h" 1
# 9 "./include/aos/mbedtls_ssl.h"
...
# 10 "./include/aos/mbedtls_ssl.h" 2
# 27 "./include/aos/mbedtls_ssl.h"
void *mbedtls_ssl_connect(void *tcp_fd, const char *ca_cert, int ca_cert_len);
# 38 "./include/aos/mbedtls_ssl.h"
int mbedtls_ssl_send(void *ssl, const char *buffer, int length);
# 49 "./include/aos/mbedtls_ssl.h"
int mbedtls_ssl_recv(void *ssl, char *buffer, int length);
# 58 "./include/aos/mbedtls_ssl.h"
int mbedtls_ssl_close(void *ssl);
# 16 "example/tls/tls_client.c" 2
# 1 "./security/mbedtls/include/mbedtls/net_sockets.h" 1
# 27 "./security/mbedtls/include/mbedtls/net_sockets.h"
# 1 "./security/mbedtls/include/mbedtls/config.h" 1
# 28 "./security/mbedtls/include/mbedtls/net_sockets.h" 2
...
void *network_socket_create(const char *net_addr, int port)
{
    ...
    do { if (0) { 
# 64 "example/tls/tls_client.c" 3
   (*__errno()) 
# 64 "example/tls/tls_client.c"
   = (0); } } while(0);
    tcp_fd = lwip_socket(2,1,0);
    if (tcp_fd < 0) {
        printf("creat socket fail - errno: %d\n", 
# 67 "example/tls/tls_client.c" 3
                                                 (*__errno())
# 67 "example/tls/tls_client.c"
                                                      );
        return 
# 68 "example/tls/tls_client.c" 3 4
              ((void *)0)
# 68 "example/tls/tls_client.c"
                  ;
    }

    host = lwip_gethostbyname(net_addr);
    if (host == 
# 72 "example/tls/tls_client.c" 3 4
               ((void *)0)
# 72 "example/tls/tls_client.c"
                   ) {
        printf("get host by name fail - errno: %d\n", 
# 73 "example/tls/tls_client.c" 3
                                                     (*__errno())
# 73 "example/tls/tls_client.c"
                                                          );
        goto _err;
    }
    ...
    lwip_setsockopt(tcp_fd,0xfff,0x1006,&opt_val,sizeof(opt_val));

    do {
        do { if (0) { 
# 85 "example/tls/tls_client.c" 3
       (*__errno()) 
# 85 "example/tls/tls_client.c"
       = (0); } } while(0);
        ret = lwip_connect(tcp_fd,(struct sockaddr*)&saddr,sizeof(saddr));
        if (ret < 0 && 
# 87 "example/tls/tls_client.c" 3
                      (*__errno()) 
# 87 "example/tls/tls_client.c"
                            != 4) {
            printf("socket connect fail - errno: %d\n", 
# 88 "example/tls/tls_client.c" 3
                                                       (*__errno())
# 88 "example/tls/tls_client.c"
                                                            );
            goto _err;
        }
    } while (ret < 0);
...
{
    if (tcp_fd == 
# 103 "example/tls/tls_client.c" 3 4
                 ((void *)0)
# 103 "example/tls/tls_client.c"
                     ) {
        return;
    }
...
# 143 "example/tls/tls_client.c"
static void tls_client_sample(void)
{
    int ret = 0;
    char buf[128];
    void *tcp_fd = 
# 147 "example/tls/tls_client.c" 3 4
                  ((void *)0)
# 147 "example/tls/tls_client.c"
                      ;
    void *ssl = 
# 148 "example/tls/tls_client.c" 3 4
               ((void *)0)
# 148 "example/tls/tls_client.c"
                   ;
    ...
    if (ssl == 
# 167 "example/tls/tls_client.c" 3 4
              ((void *)0)
# 167 "example/tls/tls_client.c"
                  ) {
...
_out:
    if (ssl != 
# 194 "example/tls/tls_client.c" 3 4
              ((void *)0)
# 194 "example/tls/tls_client.c"
                  ) {
        mbedtls_ssl_close(ssl);
    }

    if (tcp_fd != 
# 198 "example/tls/tls_client.c" 3 4
                 ((void *)0)
# 198 "example/tls/tls_client.c"
                     ) {
...
int application_start(void)
{
    ...
    aos_register_event_filter(0x0002, handle_event, cookie);

    netmgr_init();

    netmgr_start(
# 244 "example/tls/tls_client.c" 3 4
                1
# 244 "example/tls/tls_client.c"
                    );
