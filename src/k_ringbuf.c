# 1 "kernel/rhino/core/k_ringbuf.c"
...
# 6 "kernel/rhino/core/k_ringbuf.c" 2
...
kstat_t ringbuf_push(k_ringbuf_t *p_ringbuf, void *data, size_t len)
{
    ...
    if (p_ringbuf->type == 0) {
        ...
        len_bytes = sizeof(size_t);





        memcpy(c_len, &len, sizeof(size_t));
...
kstat_t ringbuf_pop(k_ringbuf_t *p_ringbuf, void *pdata, size_t *plen)
{
    ...
    if (p_ringbuf->type == 0) {
        ...
        if (plen != 
# 126 "kernel/rhino/core/k_ringbuf.c" 3 4
                   ((void *)0)
# 126 "kernel/rhino/core/k_ringbuf.c"
                       ) {
        ...
        memcpy(&len, c_len, sizeof(size_t));
        *plen = len;
...
uint8_t ringbuf_is_full(k_ringbuf_t *p_ringbuf)
{
    if (p_ringbuf->type == 1 && p_ringbuf->freesize < (sizeof(size_t) + 1)) {
        return 1;
    }
