# 1 "framework/protocol/linkkit/iotkit/base/utils//misc/utils_epoch_time.c"
# 1 "/home/stone/Documents/Ali_IOT//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "framework/protocol/linkkit/iotkit/base/utils//misc/utils_epoch_time.c"
# 19 "framework/protocol/linkkit/iotkit/base/utils//misc/utils_epoch_time.c"
...
# 20 "framework/protocol/linkkit/iotkit/base/utils//misc/utils_epoch_time.c" 2
# 1 "framework/protocol/linkkit/iotkit/base/utils//misc/utils_epoch_time.h" 1
# 26 "framework/protocol/linkkit/iotkit/base/utils//misc/utils_epoch_time.h"
...
# 27 "framework/protocol/linkkit/iotkit/base/utils//misc/utils_epoch_time.h" 2
# 36 "framework/protocol/linkkit/iotkit/base/utils//misc/utils_epoch_time.h"
uint64_t utils_get_epoch_time_from_ntp(char copy[], int len);
# 21 "framework/protocol/linkkit/iotkit/base/utils//misc/utils_epoch_time.c" 2
...
# 22 "framework/protocol/linkkit/iotkit/base/utils//misc/utils_epoch_time.c" 2
# 56 "framework/protocol/linkkit/iotkit/base/utils//misc/utils_epoch_time.c"
struct timeval_t {
    uint32_t tv_sec;
    uint32_t tv_usec;
};
...
# 87 "framework/protocol/linkkit/iotkit/base/utils//misc/utils_epoch_time.c"
...
{
    return _check_endian() ? h : ((((uint32_t)(h) & 0xff000000) >> 24) | (((uint32_t)(h) & 0x00ff0000) >> 8) | (((uint32_t)(h) & 0x0000ff00) << 8) | (((uint32_t)(h) & 0x000000ff) << 24));
}

static uint32_t _ntohl(uint32_t n)
{
    return _check_endian() ? n : ((((uint32_t)(n) & 0xff000000) >> 24) | (((uint32_t)(n) & 0x00ff0000) >> 8) | (((uint32_t)(n) & 0x0000ff00) << 8) | (((uint32_t)(n) & 0x000000ff) << 24));
}
...
static int _get_packet(unsigned char *packet, int *len)
{
    ...
    if (*len < 48) {
        LITE_syslog(__FUNCTION__, 114, LOG_ERR_LEVEL, "packet buf too short!\n");
        return -1;
    }

    memset(packet, 0, *len);

    data[0] = _htonl((0 << 30) | (3 << 27) | (3 << 24) |
                     (0 << 16) | (4 << 8) | (-6 & 0xff));
    data[1] = _htonl(1 << 16);
    data[2] = _htonl(1 << 16);
    data[10] = _htonl(now.tv_sec + 0x83aa7e80);
    data[11] = _htonl((4294 * (now.tv_usec) + ((1981 * (now.tv_usec)) >> 11)));
...
static void _rfc1305_parse_timeval(unsigned char *read_buf, struct timeval_t *tv)
{
    ...
    ntp_packet.li = _ntohl(((unsigned int *)read_buf)[0]) >> 30 & 0x03;
    ntp_packet.vn = _ntohl(((unsigned int *)read_buf)[0]) >> 27 & 0x07;
    ntp_packet.mode = _ntohl(((unsigned int *)read_buf)[0]) >> 24 & 0x07;
    ntp_packet.stratum = _ntohl(((unsigned int *)read_buf)[0]) >> 16 & 0xff;
    ntp_packet.poll = _ntohl(((unsigned int *)read_buf)[0]) >> 8 & 0xff;
    ntp_packet.prec = _ntohl(((unsigned int *)read_buf)[0]) & 0xff;
    if (ntp_packet.prec & 0x80) ntp_packet.prec |= 0xffffff00;
    ntp_packet.delay = _ntohl(((unsigned int *)read_buf)[1]);
    ntp_packet.disp = _ntohl(((unsigned int *)read_buf)[2]);
    ntp_packet.refid = _ntohl(((unsigned int *)read_buf)[3]);
# 163 "framework/protocol/linkkit/iotkit/base/utils//misc/utils_epoch_time.c"
    xmttime.coarse = _ntohl(((unsigned int *)read_buf)[10]);
    xmttime.fine = _ntohl(((unsigned int *)read_buf)[11]);
# 181 "framework/protocol/linkkit/iotkit/base/utils//misc/utils_epoch_time.c"
    tv->tv_sec = xmttime.coarse - 0x83aa7e80;
    tv->tv_usec = (((xmttime.fine) >> 12) - 759 * ((((xmttime.fine) >> 10) + 32768) >> 16));
}
...
static uint64_t _get_timestamp_from_ntp(const char *host)
{
    ...
    if (fd < 0) {
        LITE_syslog(__FUNCTION__, 197, LOG_ERR_LEVEL, "udp create error!");
        return 0;
    }

    ret = HAL_UDP_connect(fd, host, (123));
    LITE_syslog(__FUNCTION__, 202, LOG_INFO_LEVEL, "fd = HAL_UDP_connect('%s':%d) = %d", host, (123), ret);

    ret = _get_packet(write_buf, &write_len);
    if (ret < 0) {
        LITE_syslog(__FUNCTION__, 206, LOG_ERR_LEVEL, "get_packet error!");
        goto do_exit;
    }

    ret = HAL_UDP_send(fd, write_buf, write_len, 0);
    if (ret < 0) {
        LITE_syslog(__FUNCTION__, 212, LOG_ERR_LEVEL, "udp write error!");
        goto do_exit;
    }

    ret = HAL_UDP_recv(fd, read_buf, sizeof(read_buf), 3000);
    if (ret < 0) {
        LITE_syslog(__FUNCTION__, 218, LOG_ERR_LEVEL, "udp read error!");
        goto do_exit;
    }
...
uint64_t utils_get_epoch_time_from_ntp(char copy[], int len)
{
    ...
    for (ntp_server_index = 1; ntp_server_index <= 7; ntp_server_index ++) {
        HAL_Snprintf(ntp_server, 20, "ntp%d.aliyun.com", ntp_server_index);
        time_in_ms = _get_timestamp_from_ntp(ntp_server);
        if (time_in_ms > 0) {
            HAL_Snprintf(copy, len, "%" 
# 239 "framework/protocol/linkkit/iotkit/base/utils//misc/utils_epoch_time.c" 3 4
                                       "ll" 
# 239 "framework/protocol/linkkit/iotkit/base/utils//misc/utils_epoch_time.c"
                                       "u", time_in_ms);
            break;
        }
    }
...
void demo_epoch_time(void)
{
    ...
    LITE_syslog(__FUNCTION__, 255, LOG_INFO_LEVEL, "ret = utils_get_epoch_time_from_ntp() = %" 
# 255 "framework/protocol/linkkit/iotkit/base/utils//misc/utils_epoch_time.c" 3 4
   "ll" 
# 255 "framework/protocol/linkkit/iotkit/base/utils//misc/utils_epoch_time.c"
   "u", ret);
    LITE_syslog(__FUNCTION__, 256, LOG_INFO_LEVEL, "buf = '%s'", buf);
}
