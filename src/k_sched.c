...
# 6 "kernel/rhino/core/k_sched.c" 2
# 33 "kernel/rhino/core/k_sched.c"
kstat_t krhino_sched_disable(void)
{
    size_t cpsr;
    ...
    if (g_sched_lock[cpu_cur_get()] >= 200u) {
...
# 123 "kernel/rhino/core/k_sched.c"
void core_sched(void)
{
    size_t cpsr;
    uint8_t cur_cpu_num;
...
void runqueue_init(runqueue_t *rq)
{
    ...
    for (prio = 0; prio < 62; prio++) {
        rq->cur_list_item[prio] = 
# 172 "kernel/rhino/core/k_sched.c" 3 4
                                 ((void *)0)
# 172 "kernel/rhino/core/k_sched.c"
                                     ;
    }
}

static inline void ready_list_init(runqueue_t *rq, ktask_t *task)
{
...
static inline uint8_t is_ready_list_empty(uint8_t prio)
{
    return (g_ready_queue.cur_list_item[prio] == 
# 189 "kernel/rhino/core/k_sched.c" 3 4
                                                ((void *)0)
# 189 "kernel/rhino/core/k_sched.c"
                                                    );
}

static inline void _ready_list_add_tail(runqueue_t *rq, ktask_t *task)
{
...
static inline void _ready_list_add_head(runqueue_t *rq, ktask_t *task)
{
    if (is_ready_list_empty(task->prio)) {
        ready_list_init(rq, task);
        return;
...
# 261 "kernel/rhino/core/k_sched.c"
void ready_list_add_head(runqueue_t *rq, ktask_t *task)
{
...
void ready_list_rm(runqueue_t *rq, ktask_t *task)
{
    ...
    rq->cur_list_item[pri] = 
# 298 "kernel/rhino/core/k_sched.c" 3 4
                            ((void *)0)
# 298 "kernel/rhino/core/k_sched.c"
                                ;
...
# 363 "kernel/rhino/core/k_sched.c"
ktask_t *preferred_cpu_ready_task_get(runqueue_t *rq, uint8_t cpu_num)
{
    klist_t *node = rq->cur_list_item[rq->highest_pri];

    return ((ktask_t *)((uint8_t *)(node) - (size_t)(&((ktask_t *)0)->task_list)));
}
# 431 "kernel/rhino/core/k_sched.c"
void time_slice_update(void)
{
    size_t cpsr;
    ...
    task = ((ktask_t *)((uint8_t *)(head) - (size_t)(&((ktask_t *)0)->task_list)));

    if (task->sched_policy == 0u) {
        do { { cpu_intrpt_restore(cpsr); }; } while (0);
        return;
    }
