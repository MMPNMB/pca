# 1 "kernel/rhino/core/k_dyn_mem_proc.c"
...
# 6 "kernel/rhino/core/k_dyn_mem_proc.c" 2
...
void dyn_mem_proc_task(void *arg)
{
    ...
    while (1) {
        ret = krhino_sem_take(&g_res_sem, ((uint64_t)-1));
        ...
        while (1) {
            ...
            if (!is_klist_empty(&g_res_list)) {
                res_free = ((res_free_t *)((uint8_t *)(g_res_list.next) - (size_t)(&((res_free_t *)0)->res_list)));
...
void dyn_mem_proc_task_start(void)
{
    krhino_task_create(&g_dyn_task, "dyn_mem_proc_task", 0, 6,
                        0, g_dyn_task_stack, 256,
                        dyn_mem_proc_task, 1);
}
