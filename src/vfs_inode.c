...
# 6 "kernel/vfs/vfs_inode.c" 2
# 1 "./kernel/vfs/include/vfs_conf.h" 1
# 7 "kernel/vfs/vfs_inode.c" 2
# 1 "./kernel/vfs/include/vfs_err.h" 1
# 12 "./kernel/vfs/include/vfs_err.h"
...
# 13 "./kernel/vfs/include/vfs_err.h" 2
# 8 "kernel/vfs/vfs_inode.c" 2
...
# 9 "./kernel/vfs/include/vfs_inode.h" 2
...
# 10 "./kernel/vfs/include/vfs_inode.h" 2
...
# 11 "./kernel/vfs/include/vfs_inode.h" 2
...
# 12 "./kernel/vfs/include/vfs_inode.h" 2
...
# 9 "kernel/vfs/vfs_inode.c" 2




static inode_t g_vfs_dev_nodes[25];

int inode_init()
{
    memset(g_vfs_dev_nodes, 0, sizeof(inode_t) * 25);
    return 0;
}
...
int inode_alloc()
{
    ...
    return -
# 31 "kernel/vfs/vfs_inode.c" 3
           12
# 31 "kernel/vfs/vfs_inode.c"
                 ;
}
...
int inode_del(inode_t *node)
{
    if (node->refs > 0) {
        return -
# 37 "kernel/vfs/vfs_inode.c" 3
               16
# 37 "kernel/vfs/vfs_inode.c"
                    ;
    }

    if (node->refs == 0) {
        if (node->i_name != 
# 41 "kernel/vfs/vfs_inode.c" 3 4
                           ((void *)0)
# 41 "kernel/vfs/vfs_inode.c"
                               ) {
            aos_free(node->i_name);
        }

        node->i_name = 
# 45 "kernel/vfs/vfs_inode.c" 3 4
                      ((void *)0)
# 45 "kernel/vfs/vfs_inode.c"
                          ;
        node->i_arg = 
# 46 "kernel/vfs/vfs_inode.c" 3 4
                     ((void *)0)
# 46 "kernel/vfs/vfs_inode.c"
                         ;
        node->i_flags = 0;
...
inode_t *inode_open(const char *path)
{
    ...
    for (; e < 25; e++) {
        ...
        if (node == 
# 61 "kernel/vfs/vfs_inode.c" 3 4
                   ((void *)0)
# 61 "kernel/vfs/vfs_inode.c"
                       ) {
            continue;
        }
        if (node->i_name == 
# 64 "kernel/vfs/vfs_inode.c" 3 4
                           ((void *)0)
# 64 "kernel/vfs/vfs_inode.c"
                               ) {
            continue;
        }
    ...
    return 
# 78 "kernel/vfs/vfs_inode.c" 3 4
          ((void *)0)
# 78 "kernel/vfs/vfs_inode.c"
              ;
}
...
int inode_ptr_get(int fd, inode_t **node)
{
    if (fd < 0 || fd >= 25) {
        return -
# 84 "kernel/vfs/vfs_inode.c" 3
               22
# 84 "kernel/vfs/vfs_inode.c"
                     ;
    }
...
static int inode_set_name(const char *path, inode_t **inode)
{
    ...
    if (!mem) {
        return -
# 131 "kernel/vfs/vfs_inode.c" 3
               12
# 131 "kernel/vfs/vfs_inode.c"
                     ;
    }
...
int inode_reserve(const char *path, inode_t **inode)
{
    int ret;
    inode_t *node = 
# 144 "kernel/vfs/vfs_inode.c" 3 4
                   ((void *)0)
# 144 "kernel/vfs/vfs_inode.c"
                       ;

    do { if (!(path != 
# 146 "kernel/vfs/vfs_inode.c" 3 4
   ((void *)0) 
# 146 "kernel/vfs/vfs_inode.c"
   && inode != 
# 146 "kernel/vfs/vfs_inode.c" 3 4
   ((void *)0)
# 146 "kernel/vfs/vfs_inode.c"
   )) return -
# 146 "kernel/vfs/vfs_inode.c" 3
   22
# 146 "kernel/vfs/vfs_inode.c"
   ; } while(0);
    *inode = 
# 147 "kernel/vfs/vfs_inode.c" 3 4
            ((void *)0)
# 147 "kernel/vfs/vfs_inode.c"
                ;


    if (path[0] == '\0' || path[0] != '/') {
        return -
# 151 "kernel/vfs/vfs_inode.c" 3
               22
# 151 "kernel/vfs/vfs_inode.c"
                     ;
    }
...
int inode_release(const char *path)
{
    ...
    do { if (!(path != 
# 175 "kernel/vfs/vfs_inode.c" 3 4
   ((void *)0)
# 175 "kernel/vfs/vfs_inode.c"
   )) return -
# 175 "kernel/vfs/vfs_inode.c" 3
   22
# 175 "kernel/vfs/vfs_inode.c"
   ; } while(0);
    ...
    if (node == 
# 178 "kernel/vfs/vfs_inode.c" 3 4
               ((void *)0)
# 178 "kernel/vfs/vfs_inode.c"
                   ) {
        return -
# 179 "kernel/vfs/vfs_inode.c" 3
               19
# 179 "kernel/vfs/vfs_inode.c"
                     ;
    }
