...
# 7 "kernel/init/aos_init.c" 2

# 1 "./include/hal/wifi.h" 1
# 9 "./include/hal/wifi.h"
...
# 47 "./kernel/protocols/net/include/lwip/apps/tftp.h" 2
...
static void udp_cmd(char *buf, int len, int argc, char **argv)
{
    ...
    saddr.sin_family = 2;
    saddr.sin_port = lwip_htons(atoi(argv[2]));
    saddr.sin_addr.s_addr = ipaddr_addr(argv[1]);

    int sockfd = lwip_socket(2,2,0);
    if (sockfd < 0) {
        aos_cli_printf("error creating socket!\n");
        return;
    }

    int ret = lwip_sendto(sockfd,argv[3],strlen(argv[3]),0,(struct sockaddr *)&saddr,sizeof(saddr))
                                                              ;
    if (ret < 0) {
...
static void tftp_cmd(char *buf, int len, int argc, char **argv)
{
    ...
    if (strncmp(argv[1], "server", 6) == 0) {
        ...
        ip_addr_t dst_addr;
        ip4addr_aton(argc == 4 ? argv[2] : "10.0.0.2", &dst_addr);
...
# 164 "kernel/init/aos_init.c" 3 4
                             ((void *)0)
# 164 "kernel/init/aos_init.c"
                                 );
}
...
# 193 "kernel/init/aos_init.c" 3 4
                             ((void *)0)
# 193 "kernel/init/aos_init.c"
                                 , mac);
...
# 200 "kernel/init/aos_init.c" 3 4
                             ((void *)0)
# 200 "kernel/init/aos_init.c"
                                 , mac);
