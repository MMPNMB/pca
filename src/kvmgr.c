# 1 "kernel/modules/fs/kv/kvmgr.c"
...
# 6 "kernel/modules/fs/kv/kvmgr.c" 2
...
# 7 "kernel/modules/fs/kv/kvmgr.c" 2
...
# 8 "kernel/modules/fs/kv/kvmgr.c" 2
...
# 9 "kernel/modules/fs/kv/kvmgr.c" 2
# 1 "./kernel/modules/fs/kv/include/kvmgr.h" 1
# 36 "./kernel/modules/fs/kv/include/kvmgr.h"
int aos_kv_init(void);
# 46 "./kernel/modules/fs/kv/include/kvmgr.h"
void aos_kv_deinit(void);
# 10 "kernel/modules/fs/kv/kvmgr.c" 2


typedef enum {
    RES_OK = 0,
    RES_CONT = -
# 14 "kernel/modules/fs/kv/kvmgr.c" 3
                          11
# 14 "kernel/modules/fs/kv/kvmgr.c"
                                ,
    RES_NO_SPACE = -
# 15 "kernel/modules/fs/kv/kvmgr.c" 3
                          28
# 15 "kernel/modules/fs/kv/kvmgr.c"
                                ,
    RES_INVALID_PARAM = -
# 16 "kernel/modules/fs/kv/kvmgr.c" 3
                          22
# 16 "kernel/modules/fs/kv/kvmgr.c"
                                ,
    RES_MALLOC_FAILED = -
# 17 "kernel/modules/fs/kv/kvmgr.c" 3
                          12
# 17 "kernel/modules/fs/kv/kvmgr.c"
                                ,
    RES_ITEM_NOT_FOUND = -
# 18 "kernel/modules/fs/kv/kvmgr.c" 3
                          2
# 18 "kernel/modules/fs/kv/kvmgr.c"
                                ,
    RES_FLASH_READ_ERR = -
# 19 "kernel/modules/fs/kv/kvmgr.c" 3
                          5
# 19 "kernel/modules/fs/kv/kvmgr.c"
                             ,
    RES_FLASH_WRITE_ERR = -
# 20 "kernel/modules/fs/kv/kvmgr.c" 3
                          5
# 20 "kernel/modules/fs/kv/kvmgr.c"
                             ,
    RES_FLASH_EARSE_ERR = -
# 21 "kernel/modules/fs/kv/kvmgr.c" 3
                          5

# 22 "kernel/modules/fs/kv/kvmgr.c"
} result_e;
# 56 "kernel/modules/fs/kv/kvmgr.c"
...
typedef struct _kv_mgr_t {
    uint8_t kv_initialize;
    uint8_t gc_triggered;
    uint8_t gc_waiter;
    uint8_t clean_blk_nums;
    uint16_t write_pos;
    aos_sem_t gc_sem;
    aos_mutex_t kv_mutex;
    block_info_t block_info[(8192 >> 12)];
} kv_mgr_t;
...
{
    return hal_flash_read((hal_partition_t)6, &offset, buf, nbytes);
}

static int raw_write(uint32_t offset, const void *buf, size_t nbytes)
{
    return hal_flash_write((hal_partition_t)6, &offset, buf, nbytes);
}

static int raw_erase(uint32_t offset, uint32_t size)
{
    return hal_flash_erase((hal_partition_t)6, offset, size);
}
...
static void trigger_gc(void)
{
    ...
    aos_task_new("kv-gc", aos_kv_gc, 
# 147 "kernel/modules/fs/kv/kvmgr.c" 3 4
                                    ((void *)0)
# 147 "kernel/modules/fs/kv/kvmgr.c"
                                        , 1024);
}
...
{
    return raw_write(pos + 1, &state, 1);
}

static int kv_block_format(uint8_t index)
{
    block_hdr_t hdr;
    uint16_t pos = index << 12;

    memset(&hdr, 0, sizeof(hdr));
    hdr.magic = BLK_MAGIC_NUM;
    if (!raw_erase(pos, (1 << 12))) {
        hdr.state = 0xEE;
    } else {
        return RES_FLASH_EARSE_ERR;
    }

    if (raw_write(pos, &hdr, 4) != RES_OK) {
        return RES_FLASH_WRITE_ERR;
    }

    g_kv_mgr.block_info[index].state = 0xEE;
    g_kv_mgr.block_info[index].space = (1 << 12) - 4;
...
static uint16_t kv_item_calc_pos(uint16_t len)
{
    ...
    uint8_t blk_index = (g_kv_mgr.write_pos) >> 12;
    ...
    if (blk_info->space > len) {
        if (((blk_info->space - len) < (8 + 128 + 512)) && (g_kv_mgr.clean_blk_nums <= 1)) {
            trigger_gc();
        }
        return g_kv_mgr.write_pos;
    }
# 225 "kernel/modules/fs/kv/kvmgr.c"
    trigger_gc();
    return 0;
}
...
static int kv_item_del(kv_item_t *item, int mode)
{
    ...
    item_hdr_t hdr;
    char *origin_key = 
# 233 "kernel/modules/fs/kv/kvmgr.c" 3 4
                      ((void *)0)
# 233 "kernel/modules/fs/kv/kvmgr.c"
                          ;
    char *new_key = 
# 234 "kernel/modules/fs/kv/kvmgr.c" 3 4
                   ((void *)0)
# 234 "kernel/modules/fs/kv/kvmgr.c"
                       ;
    uint8_t i;
    uint16_t offset;
    ...
    if (mode == 0) {
        ...
        memset(&hdr, 0, 8);
        if (raw_read(offset, &hdr, 8) != RES_OK) {
        ...
        raw_read(offset + 8, origin_key, hdr.key_len);
        raw_read(item->pos + 8, new_key, hdr.key_len);
    ...
    if ((ret = kv_state_set(offset, 0)) != RES_OK) {
        return ret;
    }

    i = offset >> 12;
    if (g_kv_mgr.block_info[i].state == 0xCC) {
        if ((ret = kv_state_set((offset & ~((1 << 12) - 1)), 0x44)) != RES_OK) {
            return ret;
        }
        g_kv_mgr.block_info[i].state = 0x44;
    }
...
static int __item_recovery_cb(kv_item_t *item, const char *key)
{
    ...
    if (raw_read(item->pos + 8, p, item->len) != RES_OK) {
        aos_free(p);
        return RES_FLASH_READ_ERR;
    }
...
static int __item_find_cb(kv_item_t *item, const char *key)
{
    ...
    if (raw_read(item->pos + 8, item->store, item->len) != RES_OK) {
        return RES_FLASH_READ_ERR;
    }
...
static int __item_gc_cb(kv_item_t *item, const char *key)
{
    ...
    len = (8 + item->len + ~~(sizeof(void *) - 1)) & ~(sizeof(void *) - 1);
...
static kv_item_t *kv_item_traverse(item_func func, uint8_t blk_index, const char *key)
{
    kv_item_t *item;
    item_hdr_t *hdr;
    uint16_t pos = (blk_index << 12) + 4;
    uint16_t end = (blk_index << 12) + (1 << 12);
    uint16_t len = 0;
    int ret;
    ...
    do {
        ...
        if (!item) {
            return 
# 386 "kernel/modules/fs/kv/kvmgr.c" 3 4
                  ((void *)0)
# 386 "kernel/modules/fs/kv/kvmgr.c"
                      ;
        }
        memset(item, 0, sizeof(kv_item_t));
        hdr = &(item->hdr);

        if (raw_read(pos, hdr, 8) != RES_OK) {
            kv_item_free(item);
            return 
# 393 "kernel/modules/fs/kv/kvmgr.c" 3 4
                  ((void *)0)
# 393 "kernel/modules/fs/kv/kvmgr.c"
                      ;
        }
        ...
        if (hdr->val_len > 512 || hdr->key_len > 128 ||
            hdr->val_len == 0 || hdr->key_len == 0) {
            pos += 8;
            kv_item_free(item);
            if (g_kv_mgr.block_info[blk_index].state == 0xCC) {
                kv_state_set((blk_index << 12), 0x44);
                g_kv_mgr.block_info[blk_index].state = 0x44;
            }
            continue;
        }

        len = (8 + hdr->key_len + hdr->val_len + ~~(sizeof(void *) - 1)) & ~(sizeof(void *) - 1);
        ...
        if (hdr->state == 0xEE) {
            ...
            if (ret == RES_OK) {
                ...
                kv_item_free(item);
                return 
# 425 "kernel/modules/fs/kv/kvmgr.c" 3 4
                      ((void *)0)
# 425 "kernel/modules/fs/kv/kvmgr.c"
                          ;
            }
        } else {
            if (g_kv_mgr.block_info[blk_index].state == 0xCC) {
    ...
    g_kv_mgr.block_info[blk_index].space = (end > pos) ? (end - pos) : 8;
    return 
# 439 "kernel/modules/fs/kv/kvmgr.c" 3 4
          ((void *)0)
# 439 "kernel/modules/fs/kv/kvmgr.c"
              ;
}
...
static kv_item_t *kv_item_get(const char *key)
{
    ...
    for (i = 0; i < (8192 >> 12); i++) {
        if (g_kv_mgr.block_info[i].state != 0xEE) {
    ...
    return 
# 456 "kernel/modules/fs/kv/kvmgr.c" 3 4
          ((void *)0)
# 456 "kernel/modules/fs/kv/kvmgr.c"
              ;
}
...
static int kv_item_store(const char *key, const void *val, int len, uint16_t origin_off)
{
    ...
    store.len = (8 + hdr.key_len + hdr.val_len + ~~(sizeof(void *) - 1)) & ~(sizeof(void *) - 1);
...
static int kv_init(void)
{
    ...
    int ret, nums = 0;
    uint8_t i, next;
    uint8_t unclean[(8192 >> 12)] = {0};

    for (i = 0; i < (8192 >> 12); i++) {
        memset(&hdr, 0, sizeof(block_hdr_t));
        raw_read((i << 12), &hdr, 4);
        if (hdr.magic == BLK_MAGIC_NUM) {
            if ((((hdr.state) != 0xCC) && ((hdr.state) != 0xEE) && ((hdr.state) != 0x44))) {
            ...
            kv_item_traverse(__item_recovery_cb, i, 
# 551 "kernel/modules/fs/kv/kvmgr.c" 3 4
                                                   ((void *)0)
# 551 "kernel/modules/fs/kv/kvmgr.c"
                                                       );
            if (hdr.state == 0xEE) {
                if (g_kv_mgr.block_info[i].space != ((1 << 12) - 4)) {
                    unclean[nums] = i;
                    nums++;
                } else {
                    (g_kv_mgr.clean_blk_nums)++;
    ...
    while (nums > 0) {
        i = unclean[nums - 1];
        if (g_kv_mgr.clean_blk_nums >= 1) {
            if ((ret = kv_state_set((i << 12), 0x44)) != RES_OK) {
                return ret;
            }
            g_kv_mgr.block_info[i].state = 0x44;
        } else {
    ...
    if (g_kv_mgr.clean_blk_nums == (8192 >> 12)) {
        g_kv_mgr.write_pos = 4;
        if (!kv_state_set((g_kv_mgr.write_pos & ~((1 << 12) - 1)), 0xCC)) {
            g_kv_mgr.block_info[0].state = 0xCC;
            (g_kv_mgr.clean_blk_nums)--;
        }
    } else {
        for (i = 0; i < (8192 >> 12); i++) {
            if ((g_kv_mgr.block_info[i].state == 0xCC) ||
                (g_kv_mgr.block_info[i].state == 0x44)) {
                next = ((i + 1) == (8192 >> 12)) ? 0 : (i + 1);
                if (g_kv_mgr.block_info[next].state == 0xEE) {
                    g_kv_mgr.write_pos = (i << 12) + (1 << 12) - g_kv_mgr.block_info[i].space;
                    break;
                }
            }
        }
...
void aos_kv_gc(void *arg)
{
    ...
    if (aos_mutex_lock(&(g_kv_mgr.kv_mutex), 0xffffffffu) != 0) {
        goto exit;
    }
    ...
    for (gc_index = 0; gc_index < (8192 >> 12); gc_index++) {
        if (g_kv_mgr.block_info[gc_index].state == 0xEE) {
            g_kv_mgr.write_pos = (gc_index << 12) + 4;
            break;
        }
    }
    ...
    while (1) {
        ...
        if (g_kv_mgr.block_info[i].state == 0x44) {
            kv_item_traverse(__item_gc_cb, i, 
# 644 "kernel/modules/fs/kv/kvmgr.c" 3 4
                                             ((void *)0)
# 644 "kernel/modules/fs/kv/kvmgr.c"
                                                 );
            ...
            kv_state_set((g_kv_mgr.write_pos & ~((1 << 12) - 1)), 0xCC);
            g_kv_mgr.block_info[gc_index].state = 0xCC;
            (g_kv_mgr.clean_blk_nums)--;
            break;
        }
        if (i == (origin_pos >> 12)) {
...
int aos_kv_del(const char *key)
{
    kv_item_t *item;
    int ret;
    if ((ret = aos_mutex_lock(&(g_kv_mgr.kv_mutex), 0xffffffffu)) != RES_OK) {
        return ret;
    }
...
int aos_kv_set(const char *key, const void *val, int len, int sync)
{
    kv_item_t *item;
    int ret;
    if (!key || !val || len <= 0 || strlen(key) > 128 || len > 512) {
        return RES_INVALID_PARAM;
    }

    if (g_kv_mgr.gc_triggered) {
        (g_kv_mgr.gc_waiter)++;
        aos_sem_wait(&(g_kv_mgr.gc_sem), 0xffffffffu);
    }
...
{
    kv_item_t *item = 
# 727 "kernel/modules/fs/kv/kvmgr.c" 3 4
                     ((void *)0)
# 727 "kernel/modules/fs/kv/kvmgr.c"
                         ;
    int ret;
...
{
    char *p_key = 
# 763 "kernel/modules/fs/kv/kvmgr.c" 3 4
                 ((void *)0)
# 763 "kernel/modules/fs/kv/kvmgr.c"
                     ;
    char *p_val = 
# 764 "kernel/modules/fs/kv/kvmgr.c" 3 4
                 ((void *)0)
# 764 "kernel/modules/fs/kv/kvmgr.c"
                     ;
    ...
    raw_read(item->pos + 8, p_key, item->hdr.key_len);
    ...
    raw_read(item->pos + 8 + item->hdr.key_len, p_val, item->hdr.val_len);
...
static void handle_kv_cmd(char *pwbuf, int blen, int argc, char **argv)
{
    ...
    int i, ret = 0;
    char *buffer = 
# 791 "kernel/modules/fs/kv/kvmgr.c" 3 4
                  ((void *)0)
# 791 "kernel/modules/fs/kv/kvmgr.c"
                      ;
    int len = (1 << 12);
    ...
    if (strcmp(rtype, "set") == 0) {
        ...
        for (i = 0; i < (8192 >> 12); i++) {
            kv_item_traverse(__item_print_cb, i, 
# 834 "kernel/modules/fs/kv/kvmgr.c" 3 4
                                                ((void *)0)
# 834 "kernel/modules/fs/kv/kvmgr.c"
                                                    );
        }
    }
    return;
...
int aos_kv_init(void)
{
    ...
    if ((8192 >> 12) <= 1) {
        return -
# 857 "kernel/modules/fs/kv/kvmgr.c" 3
               22
# 857 "kernel/modules/fs/kv/kvmgr.c"
                     ;
    }
    ...
    blk_index = (g_kv_mgr.write_pos >> 12);
    if (((g_kv_mgr.block_info[blk_index].space) < (8 + 128 + 512)) &&
        (g_kv_mgr.clean_blk_nums < 1 + 1)) {
        trigger_gc();
    }
