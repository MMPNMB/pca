# 1 "kernel/protocols/net/apps/tftp/tftp_client.c"
...
# 6 "kernel/protocols/net/apps/tftp/tftp_client.c" 2
...
# 7 "kernel/protocols/net/apps/tftp/tftp_client.c" 2
...
# 9 "kernel/protocols/net/apps/tftp/tftp_client.c" 2
...
# 11 "kernel/protocols/net/apps/tftp/tftp_client.c" 2
...
# 12 "kernel/protocols/net/apps/tftp/tftp_client.c" 2



# 14 "kernel/protocols/net/apps/tftp/tftp_client.c"
...
# 42 "kernel/protocols/net/apps/tftp/tftp_client.c" 3 4
                           ((void *)0)
# 42 "kernel/protocols/net/apps/tftp/tftp_client.c"
                               );
    ...
    if (pstate->handle) {
        ...
        pstate->handle = 
# 47 "kernel/protocols/net/apps/tftp/tftp_client.c" 3 4
                        ((void *)0)
# 47 "kernel/protocols/net/apps/tftp/tftp_client.c"
                            ;
    }
    ...
    if (pstate->cb != 
# 51 "kernel/protocols/net/apps/tftp/tftp_client.c" 3 4
                     ((void *)0)
# 51 "kernel/protocols/net/apps/tftp/tftp_client.c"
                         )
...
# 61 "kernel/protocols/net/apps/tftp/tftp_client.c" 3 4
                            ((void *)0)
# 61 "kernel/protocols/net/apps/tftp/tftp_client.c"
                                ) {
        return;
    }

    sys_timeout(50, tftp_tmr, 
# 65 "kernel/protocols/net/apps/tftp/tftp_client.c" 3 4
                                           ((void *)0)
# 65 "kernel/protocols/net/apps/tftp/tftp_client.c"
                                               );

    if ((pstate->tick - pstate->last_tick) > (1000 / 50)) {
        if ((pstate->seq_expect > 1) && (pstate->retries < 5)) {
...
{
    tftp_state_t *pstate = &tftp_state;
    if (pstate->seq_expect == 0 && ((&pstate->addr)->addr == (addr)->addr)){
        pstate->port = port;
        pstate->seq_expect = 1;
    }

    if ( port != pstate->port || !((&pstate->addr)->addr == (addr)->addr) ) {
    ...
    uint16_t opcode = ((((sbuf[0]) & 0xff) << 8) | (((sbuf[0]) & 0xff00) >> 8));
    uint16_t blknum, blklen;
    switch (opcode) {
        case 3:
            blknum = ((((sbuf[1]) & 0xff) << 8) | (((sbuf[1]) & 0xff00) >> 8));
            blklen = p->tot_len - 4;
            if (blknum < pstate->seq_expect) {
                do { if ( ((0x80U | 0x20U) & 0x80U) && ((0x80U | 0x20U) & 0x80U) && ((s16_t)((0x80U | 0x20U) & 0x03) >= 0x00)) { do {bk_printf ("received repeated block '%d', len='%u'\n", blknum, blklen) ;} while(0); if ((0x80U | 0x20U) & 0x08U) { while(1); } } } while(0)
                                                                                     ;
            ...
            if (blknum > pstate->seq_expect) {
                do { if ( ((0x80U | 0x20U) & 0x80U) && ((0x80U | 0x20U) & 0x80U) && ((s16_t)((0x80U | 0x20U) & 0x03) >= 0x00)) { do {bk_printf ("received error block '%d', len='%u'\n", blknum, blklen ) ;} while(0); if ((0x80U | 0x20U) & 0x08U) { while(1); } } } while(0)
                                                                                   ;
                tftp_send_error(pstate->upcb, addr, port,
            ...
            do { if ( ((0x80U | 0x20U) & 0x80U) && ((0x80U | 0x20U) & 0x80U) && ((s16_t)((0x80U | 0x20U) & 0x03) >= 0x00)) { do {bk_printf ("received new block '%d', len='%u'\n", blknum, blklen) ;} while(0); if ((0x80U | 0x20U) & 0x08U) { while(1); } } } while(0)
                                                                            ;
            pbuf_header(p, -4);
            int wlen = pstate->ctx->write(pstate->handle, p);
            if (wlen != blklen) {
                tftp_send_error(pstate->upcb, addr, port, TFTP_ERROR_DISK_FULL, "disk full");
                do { if ( ((0x80U | 0x20U) & 0x80U) && ((0x80U | 0x20U) & 0x80U) && ((s16_t)((0x80U | 0x20U) & 0x03) >= 0x00)) { do {bk_printf ("write block failed\n") ;} while(0); if ((0x80U | 0x20U) & 0x08U) { while(1); } } } while(0);
                close_handle(-1);
                break;
            }
            ...
            if (blklen < 512) {
                ...
                do { if ( ((0x80U | 0x20U) & 0x80U) && ((0x80U | 0x20U) & 0x80U) && ((s16_t)((0x80U | 0x20U) & 0x03) >= 0x00)) { do {bk_printf ("get succeed: received %u bytes in %u mS\n", pstate->flen, pstate->time) ;} while(0); if ((0x80U | 0x20U) & 0x08U) { while(1); } } } while(0)
                                                                                                  ;
                close_handle(0);
                break;
            }
            break;
        case 5:
            do { if ( ((0x80U | 0x20U) & 0x80U) && ((0x80U | 0x20U) & 0x80U) && ((s16_t)((0x80U | 0x20U) & 0x03) >= 0x00)) { do {bk_printf ("sever return error '%d', msg: '%s'\n", ((((sbuf[1]) & 0xff) << 8) | (((sbuf[1]) & 0xff00) >> 8)), &sbuf[2]) ;} while(0); if ((0x80U | 0x20U) & 0x08U) { while(1); } } } while(0)
                                                                                          ;
            close_handle(-1);
            break;
        default:
...
{
    FILE *fp = 
# 158 "kernel/protocols/net/apps/tftp/tftp_client.c" 3 4
              ((void *)0)
# 158 "kernel/protocols/net/apps/tftp/tftp_client.c"
                  ;
                    ...
                    tftp_context_t *ctx, tftp_done_cb cb)
{
    ...
    if (pstate->handle == 
# 205 "kernel/protocols/net/apps/tftp/tftp_client.c" 3 4
                         ((void *)0)
# 205 "kernel/protocols/net/apps/tftp/tftp_client.c"
                             ) {
        do { if ( ((0x80U | 0x20U) & 0x80U) && ((0x80U | 0x20U) & 0x80U) && ((s16_t)((0x80U | 0x20U) & 0x03) >= 0x00)) { do {bk_printf ("error: open file '%s' failed\n", fname) ;} while(0); if ((0x80U | 0x20U) & 0x08U) { while(1); } } } while(0);
        return -1;
    }

    struct udp_pcb *pcb = udp_new_ip_type(IPADDR_TYPE_ANY);
    if (pcb == 
# 211 "kernel/protocols/net/apps/tftp/tftp_client.c" 3 4
              ((void *)0)
# 211 "kernel/protocols/net/apps/tftp/tftp_client.c"
                  ) {
        return ERR_MEM;
    }
    ...
    if (ret != ERR_OK) {
        do { if ( ((0x80U | 0x20U) & 0x80U) && ((0x80U | 0x20U) & 0x80U) && ((s16_t)((0x80U | 0x20U) & 0x03) >= 0x00)) { do {bk_printf ("error: bind to port '%u' failed\n", port) ;} while(0); if ((0x80U | 0x20U) & 0x08U) { while(1); } } } while(0);
        udp_remove(pcb);
        return ret;
    }
    ...
    if (p == 
# 226 "kernel/protocols/net/apps/tftp/tftp_client.c" 3 4
            ((void *)0)
# 226 "kernel/protocols/net/apps/tftp/tftp_client.c"
                ) {
        do { if ( ((0x80U | 0x20U) & 0x80U) && ((0x80U | 0x20U) & 0x80U) && ((s16_t)((0x80U | 0x20U) & 0x03) >= 0x00)) { do {bk_printf ("error: alloc pbuf failed\n") ;} while(0); if ((0x80U | 0x20U) & 0x08U) { while(1); } } } while(0);
        udp_remove(pcb);
        return ERR_MEM;
    }
    ...
    if (ret != ERR_OK) {
        do { if ( ((0x80U | 0x20U) & 0x80U) && ((0x80U | 0x20U) & 0x80U) && ((s16_t)((0x80U | 0x20U) & 0x03) >= 0x00)) { do {bk_printf ("error: send RRQ to server failed\n") ;} while(0); if ((0x80U | 0x20U) & 0x08U) { while(1); } } } while(0);
        pbuf_free(p);
        udp_remove(pcb);
        return ERR_MEM;
    }
    ...
    udp_recv(pcb, recv, 
# 247 "kernel/protocols/net/apps/tftp/tftp_client.c" 3 4
                       ((void *)0)
# 247 "kernel/protocols/net/apps/tftp/tftp_client.c"
                           );
    pstate->tick = 0;
    pstate->last_tick = 0;
    sys_timeout(50, tftp_tmr, 
# 250 "kernel/protocols/net/apps/tftp/tftp_client.c" 3 4
                                           ((void *)0)
# 250 "kernel/protocols/net/apps/tftp/tftp_client.c"
                                               );
    pstate->upcb = pcb;
    pstate->ctx = ctx;
    pstate->cb = cb;
