# 1 "kernel/rhino/core/k_mm_debug.c"
...
# 6 "kernel/rhino/core/k_mm_debug.c" 2
...
# 8 "kernel/rhino/core/k_mm_debug.c" 2
...
# 16 "kernel/rhino/core/k_mm_debug.c" 2
...
# 17 "kernel/rhino/core/k_mm_debug.c" 2
extern int csp_printf(const char *fmt, ...);
# 26 "kernel/rhino/core/k_mm_debug.c"
extern klist_t g_mm_region_list_head;




static mm_scan_region_t g_mm_scan_region[10];
...
uint32_t krhino_mm_leak_region_init(void *start, void *end)
{
    ...
    if ((start == 
# 47 "kernel/rhino/core/k_mm_debug.c" 3 4
                 ((void *)0)
# 47 "kernel/rhino/core/k_mm_debug.c"
                     ) || (end == 
# 47 "kernel/rhino/core/k_mm_debug.c" 3 4
                                  ((void *)0)
# 47 "kernel/rhino/core/k_mm_debug.c"
...
static uint32_t check_if_in_stack(void **p)
{
    ...
    for (tmp = taskhead->next; tmp != taskend; tmp = tmp->next) {
        task = ((ktask_t *)((uint8_t *)(tmp) - (size_t)(&((ktask_t *)0)->task_stats_item)));
...
uint32_t scan_region(void *start, void *end, void *adress)
{
    ...
    while ((void *)p < end) {
        if (
# 108 "kernel/rhino/core/k_mm_debug.c" 3 4
           ((void *)0) 
# 108 "kernel/rhino/core/k_mm_debug.c"
                != p && adress == *p) {
            g_leak_match = p;
            return 1;
        }
...
uint32_t check_mm_leak(uint8_t *adress)
{
    ...
    for (i = 0; i < 10; i++) {
        ...
        if ((
# 125 "kernel/rhino/core/k_mm_debug.c" 3 4
            ((void *)0) 
# 125 "kernel/rhino/core/k_mm_debug.c"
                 == g_mm_scan_region[i].start) || (
# 125 "kernel/rhino/core/k_mm_debug.c" 3 4
                                                   ((void *)0) 
# 125 "kernel/rhino/core/k_mm_debug.c"
                                                        == g_mm_scan_region[i].end)) {
            continue;
        }
...
static uint32_t recheck(void *start, void *end)
{
    ...
    while ((void *)p <= end) {
        if (
# 150 "kernel/rhino/core/k_mm_debug.c" 3 4
           ((void *)0) 
# 150 "kernel/rhino/core/k_mm_debug.c"
                != p && 1 == if_adress_is_valid(*p)) {
...
uint32_t check_malloc_region(void *adress)
{
    ...
    do { if (g_kmm_head == 
# 170 "kernel/rhino/core/k_mm_debug.c" 3 4
   ((void *)0)
# 170 "kernel/rhino/core/k_mm_debug.c"
   ) { return RHINO_NULL_PTR; } } while (0);

    reginfo = g_kmm_head->regioninfo;
    while (reginfo) {
        cur = ((k_mm_list_t *)((char *)(reginfo) - ((((sizeof(k_mm_list_t) - sizeof(free_ptr_t)) + ((1<<3)-1)) & ~((1<<3)-1)))));

        cur = ((k_mm_list_t *)((cur)->mbinfo.buffer + ((cur)->buf_size & (~((1<<3)-1)))));
        while (cur) {
            if (((cur)->buf_size & (~((1<<3)-1)))) {
                next = ((k_mm_list_t *)((cur)->mbinfo.buffer + ((cur)->buf_size & (~((1<<3)-1)))));
                if (0 == g_recheck_flag && !(cur->buf_size & 1)) {
                ...
                next = 
# 192 "kernel/rhino/core/k_mm_debug.c" 3 4
                      ((void *)0)
# 192 "kernel/rhino/core/k_mm_debug.c"
                          ;
            }
            if (1 == g_recheck_flag &&
    ...
    while (reginfo) {
        ...
        while (cur) {
            if (((cur)->buf_size & (~((1<<3)-1)))) {
                ...
                next = 
# 229 "kernel/rhino/core/k_mm_debug.c" 3 4
                      ((void *)0)
# 229 "kernel/rhino/core/k_mm_debug.c"
                          ;
            }
            cur = next;
        }
...
uint32_t dump_mmleak()
{
    ...
    do { kspinlock_t *s = (kspinlock_t *)(&(g_kmm_head->mm_lock)); s->cpsr = cpu_intrpt_save(); } while (0);;
        ...
        while (cur) {
            if (((cur)->buf_size & (~((1<<3)-1)))) {
                ...
                if (!(cur->buf_size & 1) &&
                    ...
                    aos_cli_printf("adress:0x%0x owner:0x%0x len:%-5d type:%s\r\n",
                          (void *)cur->mbinfo.buffer, cur->owner,
                          ((cur)->buf_size & (~((1<<3)-1))), "leak");
                }

            } else {
                next = 
# 265 "kernel/rhino/core/k_mm_debug.c" 3 4
                      ((void *)0)
# 265 "kernel/rhino/core/k_mm_debug.c"
                          ;
            }
            cur = next;
        }
    ...
    do { kspinlock_t *s = (kspinlock_t *)(&(g_kmm_head->mm_lock)); cpu_intrpt_restore(s->cpsr); } while (0);;
...
void print_block(k_mm_list_t *b)
{
    ...
    if (((b)->buf_size & (~((1<<3)-1)))) {
        aos_cli_printf(" %6lu ", (unsigned long)((b)->buf_size & (~((1<<3)-1))));
    } else {
        aos_cli_printf(" sentinel ");
    }


    aos_cli_printf(" %8x ", b->dye);
    aos_cli_printf(" 0x%-8x ", b->owner);


    if (b->buf_size & 2) {
        aos_cli_printf("pre-free [%8p];", b->prev);
    } else {
        aos_cli_printf("pre-used;");
    }

    if (b->buf_size & 1) {
        aos_cli_printf(" free[%8p,%8p] ", b->mbinfo.free_ptr.prev, b->mbinfo.free_ptr.next);
    }
    aos_cli_printf("\r\n");
...
void dump_kmm_free_map(k_mm_head *mmhead)
{
    ...
    aos_cli_printf("freelist bitmap: 0x%x\r\n", (unsigned)mmhead->free_bitmap);
    aos_cli_printf("address,  stat   size     dye     caller   pre-stat    point\r\n");
    ...
    while (reginfo) {
        ...
        while (cur) {
            ...
            if (((cur)->buf_size & (~((1<<3)-1)))) {
                ...
                next = 
# 373 "kernel/rhino/core/k_mm_debug.c" 3 4
                      ((void *)0)
# 373 "kernel/rhino/core/k_mm_debug.c"
                          ;
            }
            cur = next;
        }
...
void dump_kmm_statistic_info(k_mm_head *mmhead)
{
    ...
    aos_cli_printf("     free     |     used     |     maxused\r\n");
    aos_cli_printf("  %10d  |  %10d  |  %10d\r\n",
          mmhead->free_size, mmhead->used_size, mmhead->maxused_size);
    aos_cli_printf("\r\n");
    aos_cli_printf("-----------------number of alloc times:-----------------\r\n");
    for (i = 0; i < (19 - 6 + 2); i++) {
        if (i % 4 == 0 && i != 0) {
            aos_cli_printf("\r\n");
        }
        aos_cli_printf("[2^%02d] bytes: %5d   |", (i+6), mmhead->alloc_times[i]);
    }
    aos_cli_printf("\r\n");
    ...
    if ( pool != 
# 410 "kernel/rhino/core/k_mm_debug.c" 3 4
                ((void *)0) 
# 410 "kernel/rhino/core/k_mm_debug.c"
                     )
    {
        aos_cli_printf("-----------------fix pool information:-----------------\r\n");
        aos_cli_printf("     free     |     used     |     total\r\n");
        aos_cli_printf("  %10d  |  %10d  |  %10d\r\n",
              pool->blk_avail*32,
              (pool->blk_whole - pool->blk_avail)*32,
              pool->blk_whole*32);
    }
...
uint32_t dumpsys_mm_info_func(uint32_t len)
{
    ...
    aos_cli_printf("\r\n");
    aos_cli_printf("------------------------------- all memory blocks --------------------------------- \r\n");
    aos_cli_printf("g_kmm_head = %8x\r\n", (unsigned int)g_kmm_head);

    dump_kmm_map(g_kmm_head);
    aos_cli_printf("\r\n");
    aos_cli_printf("----------------------------- all free memory blocks ------------------------------- \r\n");
    dump_kmm_free_map(g_kmm_head);
    aos_cli_printf("\r\n");
    aos_cli_printf("------------------------- memory allocation statistic ------------------------------ \r\n");
