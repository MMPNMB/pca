...
void print_block(k_mm_list_t *b)
{
    ...
    if (((b)->buf_size & (~((1<<3)-1)))) {
        printf(" %6lu ", (unsigned long)((b)->buf_size & (~((1<<3)-1))));
    } else {
        printf(" sentinel ");
    }
...
void dump_kmm_statistic_info(k_mm_head *mmhead)
{
    ...
    printf("\r\n");
    printf("-----------------number of alloc times:-----------------\r\n");
    for (i = 0; i < (19 - 6 + 2); i++) {
        if (i % 4 == 0 && i != 0) {
            printf("\r\n");
        }
        printf("[2^%02d] bytes: %5d   |", (i+6), mmhead->alloc_times[i]);
    }
    printf("\r\n");
...
# 410 "kernel/rhino/core/k_mm_debug.c"
    ...
    {
        printf("-----------------fix pool information:-----------------\r\n");
        printf("     free     |     used     |     total\r\n");
        printf("  %10d  |  %10d  |  %10d\r\n",
              pool->blk_avail*32,
